 ...
 * @param {string} trait  Trait as defined in `CONFIG.TRESDETV.traits`.
 * @returns {object}      Trait categories defined within `CONFIG.TRESDETV`.
 */
export function categories(trait) {
	const traitConfig = CONFIG.TRESDETV.traits[trait];
	return CONFIG.TRESDETV[traitConfig.configKey ?? trait];
}
...
export async function choices(trait, chosen = new Set()) {
	...
	if (traitConfig.children) {
		for (const [categoryKey, childrenKey] of Object.entries(traitConfig.children)) {
			const children = CONFIG.TRESDETV[childrenKey];
	...
	if (traitConfig.subtypes) {
		...
		const map = CONFIG.TRESDETV[`${trait}ProficienciesMap`];

		// Merge all IDs lists together
		const ids = traitConfig.subtypes.ids.reduce((obj, key) => {
			if (CONFIG.TRESDETV[key]) Object.assign(obj, CONFIG.TRESDETV[key]);
			return obj;
		}, {});
	...
	// Sort Categories
	if (traitConfig.sortCategories) data = tresdetv.utils.sortObjectEntries(data, "label");

	// Sort Children
	for (const category of Object.values(data)) {
		if (!category.children) continue;
		category.children = tresdetv.utils.sortObjectEntries(category.children, "label");
	}
...
export function getBaseItem(identifier, { indexOnly = false, fullItem = false } = {}) {
	let pack = CONFIG.TRESDETV.sourcePacks.ITEMS;
...
export function traitIndexFields() {
	const fields = [];
	for (const traitConfig of Object.values(CONFIG.TRESDETV.traits)) {
...
export function traitLabel(trait, count) {
	...
	return game.i18n.localize(`TRESDETV.Trait${typeCap}Plural.${pluralRule}`);
}
...
export function keyLabel(trait, key) {
	...
	for (const childrenKey of Object.values(traitConfig.children ?? {})) {
		if (CONFIG.TRESDETV[childrenKey]?.[key]) return CONFIG.TRESDETV[childrenKey]?.[key];
	}
...
export function choiceLabel(trait, choice) {
	...
	if (!choice.pool) {
		return game.i18n.format("TRESDETV.TraitConfigChooseAny", {
			count: choice.count,
	...
	return game.i18n.format("TRESDETV.TraitConfigChooseList", {
		count: choice.count,
